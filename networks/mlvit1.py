...
class resnet_block(nn.Module):
    def __init__(self,in_ch,embed_dim,patch_size):
        ...
        self.conv1 = nn.Conv2d(in_ch,embed_dim,1,1)
        self.conv2 = nn.Conv2d(embed_dim,embed_dim,kernel_size = patch_size,stride=patch_size,groups=embed_dim)
        self.conv3 = nn.Conv2d(embed_dim,embed_dim,1,1)
        self.avgpool = F.avg_pool2d
        self.conv4 = nn.Conv2d(in_ch,embed_dim,1,1)
    ...
    def forward(self,input):
        ...
        x2 = self.avgpool(input,self.patch_size,self.patch_size)
        x2 = self.conv4(x2)
...
class LeFF(nn.Module):
    ...
    def __init__(self, h,w,dim = 192, scale = 4, depth_kernel = 3):
        super().__init__()
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1,use_local = False,width = None):
        super().__init__()
        ...
        self.conv_mpl = Residual(PreNorm(dim, LeFF(self.h,self.w,dim, mlp_ratio)))
...
class MultilayerVisonTransformer(nn.Module):
    ...
    """
    def __init__(self,img_size=224, patch_size=2, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 num_heads=[1, 2, 4, 8], mlp_ratios=[8, 8, 4, 4], qkv_bias=True, qk_scale=None, drop_rate=0.,
                 attn_drop_rate=0., drop_path_rate=0., norm_layer=nn.LayerNorm,
                 depths=[3, 4, 12, 3], sr_ratios=[8, 4, 2, 1]):
        ...
        self.block1 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[0],width=112)
