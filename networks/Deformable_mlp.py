...
class get_rate(nn.Module):
    """
    paramter C indicate input channel
    """
    def __init__(self,C):
        super(get_rate,self).__init__()
        self.gp = nn.AdaptiveAvgPool2d(output_size = (1,1))
        self.conv1 = nn.Conv2d(C,C//8,1,bias = False)
        self.conv2 = nn.Conv2d(C//8,C,1,bias = False)
        self.fc = nn.Sequential(self.conv1,nn.BatchNorm2d(C//8),nn.ReLU(inplace = True),
                                self.conv2)   
    ...
    def forward(self,x):
        offset = self.fc(self.gp(x))
        return offset     

class CycleFC(nn.Module):
    """
    here is our deformable fc
    """
    ...
    def __init__(
        ...
        # self.FC1 = nn.Conv2d(in_channels,self.in_channels,3,1,1,bias=True)
        # self.FC2 = nn.Conv2d(in_channels,self.in_channels,3,1,1,bias=True)
        # self.FC1 = nn.Conv2d(in_channels,2,1,1,bias=True)
        # self.FC2 = nn.Conv2d(in_channels,2,1,1,bias=True)

        self.mask = False
        if self.mask:
            self.m_conv = nn.Conv2d(self.in_channels, self.in_channels*2, kernel_size=3, padding=1, stride=stride)
    ...
    def rate_offset(self,input,compute_offset):
        B,C,H,W = input.shape
        offset = torch.empty(B, self.in_channels*2, 1, 1)
        for i in range(self.in_channels):
            if self.kernel_size[0] == 1:
                offset[:, 2 * i + 0, 0, 0] = 0
                offset[:, 2 * i + 1, 0, 0] = compute_offset[:,i,0,0]
            else:
                offset[:, 2 * i + 0, 0, 0] = compute_offset[:,i,0,0]
                offset[:, 2 * i + 1, 0, 0] = 0
        return offset
    ...
    def forward(self, input: Tensor,compute_offset=None):
        """
        Args:
        ...
        B, C, H, W = input.size()
        if self.use_deform:
            offset = self.rate_offset(input,compute_offset = compute_offset)
            offset = offset.cuda()
            return deform_conv2d_tv(input, offset.expand(B, -1, H, W), self.weight, self.bias, stride=self.stride,
...
class CycleMLP(nn.Module):
    def __init__(self, dim, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.,use_deform = False):
        super(CycleMLP,self).__init__()
        self.mlp_c = nn.Linear(dim, dim, bias=qkv_bias)

        self.get_offset = get_rate(dim)
        self.sfc_h = CycleFC(dim, dim, (1, 3), 1, 0,use_deform = use_deform)
        self.sfc_w = CycleFC(dim, dim, (3, 1), 1, 0,use_deform = use_deform)
    ...
    def forward(self, x):
        B, H, W, C = x.shape  #[1,56,56,64]
      #  print('x shape:',x.shape)
        offset = self.get_offset(x.permute(0, 3, 1, 2))
        h = self.sfc_h(x.permute(0, 3, 1, 2),compute_offset = offset).permute(0, 2, 3, 1)
        w = self.sfc_w(x.permute(0, 3, 1, 2),compute_offset = offset).permute(0, 2, 3, 1)
        c = self.mlp_c(x)
...
class CycleBlock(nn.Module):
    ...
    def __init__(self, dim, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, skip_lam=1.0, mlp_fn=CycleMLP,use_deform = False):
        super(CycleBlock,self).__init__()
        self.norm1 = norm_layer(dim)
        self.attn = mlp_fn(dim, qkv_bias=qkv_bias, qk_scale=None, attn_drop=attn_drop,use_deform = use_deform)
...
class PatchEmbedOverlapping(nn.Module):
    ...
    def __init__(self, patch_size=16, stride=16, padding=0, in_chans=3, embed_dim=768, norm_layer=None, groups=1):
        ...
        self.proj = nn.Conv2d(96, embed_dim, kernel_size=patch_size, stride=stride, padding=padding, groups=groups)
...
#            use_deform = False
        ...
        blocks.append(CycleBlock(dim, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                      attn_drop=attn_drop, drop_path=block_dpr, skip_lam=skip_lam, mlp_fn=mlp_fn,use_deform = use_deform))
...
class CycleNet(nn.Module):
    ...
    def __init__(self, layers, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        self.patch_embed = PatchEmbedOverlapping(patch_size=7, stride=2, padding=3, in_chans=3, embed_dim=embed_dims[0])
...
class CycleMLP_B5_feat(CycleNet):
   def __init__(self, **kwargs):
       ...
       super(CycleMLP_B5_feat, self).__init__(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                                              mlp_ratios=mlp_ratios, skip_lam = 2.,mlp_fn=CycleMLP, fork_feat=True)
...
class CycleMLP_B4_feat(CycleNet):
   def __init__(self, **kwargs):
       ...
       super(CycleMLP_B4_feat, self).__init__(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                                              mlp_ratios=mlp_ratios, skip_lam = 3.,mlp_fn=CycleMLP, fork_feat=True)
...
def CycleMLP_B5(pretrained=False, **kwargs):
    ...
    embed_dims = [96, 192, 384, 768]
    model = CycleNet(layers, embed_dims=embed_dims, patch_size=7, transitions=transitions,
                     mlp_ratios=mlp_ratios, skip_lam = 2.,mlp_fn=CycleMLP, **kwargs)
    model.default_cfg = default_cfgs['cycle_L']
    return model
