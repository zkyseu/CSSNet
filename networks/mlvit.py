...
from .MLP import external_attention2
#from .MLP import GLAttention
#
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1,kernel_size=3,
                 ...
                 use_external = None,dim_n = None,use_fc = None,ws = None):
        super().__init__()
        ...
        #self.GLattn = GLAttention(dim,dim_n,ws = ws)
        self.external_attention2 = external_attention2(dim,dim_n)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 ...
                 width = None,use_external = None,dim_n = None,use_fc = False,use_deform = False,ws = None):
        super().__init__()
        ...
        self.attn = Attention(
            ...
            use_external=use_external,dim_n=dim_n,use_fc=use_fc,ws = ws)
...
class MultilayerVisonTransformer(nn.Module):
    ...
    def __init__(self,img_size=128, patch_size=2, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[3, 4, 18, 3], sr_ratios=[8, 4, 2, 1],dim_n = [64*64,32*32,16*16,8*8]):
        ...
        self.block1 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[0],width=64,use_external=True,dim_n=dim_n[0],ws=1 if i % 2 == 1 else 8)
        ...
        self.block2 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[1],width=32,use_external=True,dim_n = dim_n[1],ws=1 if i % 2 == 1 else 8)
        ...
        self.block3 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[2],width=16,use_external=True,dim_n=dim_n[2],ws=1 if i % 2 == 1 else 8)
        ...
        self.block4 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[3],width=8,use_external=True,dim_n=dim_n[3],ws=1 if i % 2 == 1 else 8)
    ...
    def forward_features(self, x):
        ...
        for i,blk in enumerate(self.block1):
            ...
            if i==0:
                x = self.peg1(x,H,W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous() #[b,64,64,64]
        s1 = x
        ...
        for i,blk in enumerate(self.block2):
            ...
            if i==0:
                x = self.peg2(x,H,W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous() #[b,128,32,32]
        s2 = x
        ...
        for i,blk in enumerate(self.block3):
            ...
            if i==0:
                x = self.peg3(x,H,W)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous() #[b,256,16,16]
        s3 = x
        # stage 4
        ...
        x = self.norm(x)
        x = x.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous() #[b,512,8,8]
        s4 = x
