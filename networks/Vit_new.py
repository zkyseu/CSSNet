...
class Convmpl(nn.Module):
    """
    this is our conv mlp
    in this mlp,the length of token is not changed
    scale 相当于一个patch size
    """
    def __init__(self,dim = 768,scale = 4,depth_kernel = 3):
        super(Convmpl,self).__init__()
        scale_dim = dim*scale
...
class DecoderCup(nn.Module):
    def __init__(self, config):
        ...
        self.single_block = DecoderBlock(512,256,256,use_enhance=True)
        blocks = [
    ...
    def forward(self, hidden_states, features=None):
        ...
        B,hidden,h,w = hidden_states.size()
        x = hidden_states
        ...
        for i, decoder_block in enumerate(self.blocks):
            if features is not None:
                ...
                skip = None
#            if i==0:
#                x = self.single_block(x, skip=skip)
#            else:
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        #print('ddddd cccc',config['decoder_channels'])
    ...
    def forward(self, x):
        ...
        x,feats = self.decoder(x, features)
