...
class MultilayerVisonTransformer(nn.Module):
    ...
    def __init__(self,img_size=224, patch_size=4, in_chans=3, num_classes=1000, embed_dims=[64, 128, 256, 512],
                 ...
                 depths=[2, 2, 4, 2], sr_ratios=[8, 4, 2, 1],dim_n = [56*56,28*28,14*14,7*7]):
        ...
        self.block1 = nn.ModuleList([Block(
            ...
            sr_ratio=sr_ratios[0],width=56,use_external=True,dim_n=dim_n[0],use_fc = True)
        ...
        self.block2 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[1],width=28,use_external=True,dim_n = dim_n[1],use_fc = True)
        ...
        self.block3 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[2],width=14,use_external=True,dim_n=dim_n[2],use_fc = True)
        ...
        self.block4 = nn.ModuleList([Block(
            ...
            drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + i], norm_layer=norm_layer,
            sr_ratio=sr_ratios[3],width=7,use_external=True,dim_n=dim_n[3],use_fc = True)
...
class MLViTSeg(nn.Module):
    def __init__(self,config):
        ...
        self.up = nn.ModuleList([nn.UpsamplingBilinear2d(scale_factor=2) for i in range(3)])
...
#            nn.ReLU(),)
    ...
    def forward(self,img):
        ...
        for i in range(len(features)):
            features[i] = self.up[i](features[i])
        features = features[::-1]

        x,feats = self.decoder(self.up0(out_vit[-1]), features) 
