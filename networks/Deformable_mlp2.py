...
class CycleFC(nn.Module):
    ...
    def __init__(
        ...
        self.FC1 = nn.Conv2d(in_channels,self.in_channels,(3,1),(1,0))
        self.FC2 = nn.Conv2d(in_channels,self.in_channels,(1,3),(0,1))

        self.mask = True
        if self.mask:
            self.m_conv = nn.Conv2d(self.in_channels, self.in_channels, kernel_size=3, padding=1, stride=stride)
    ...
    def get_offset(self,input):
        ...
        if self.kernel_size[0]==1:
            self.offsetfc = self.FC2
        else:
            self.offsetfc = self.FC1
        out_offset = self.offsetfc(input)
        out_offset = torch.nn.functional.adaptive_avg_pool2d(out_offset, (1,1))
        offset = torch.empty(1, self.in_channels*2, 1, 1)
        for i in range(self.in_channels):
            if self.kernel_size[0]==1:
                offset[0, 2 * i + 0, 0, 0] = 0
                offset[0, 2 * i + 1, 0, 0] = out_offset[i]
            else:
                offset[0, 2 * i + 0, 0, 0] = out_offset[i]
    ...
    def gen_offset(self):
        ...
        """
        offset = torch.empty(1, self.in_channels*2, 1, 1)
        offset_value = nn.Parameter(torch.empty(self.in_channels))
        start_idx = (self.kernel_size[0] * self.kernel_size[1]) // 2
        assert self.kernel_size[0] == 1 or self.kernel_size[1] == 1, self.kernel_size
        for i in range(self.in_channels):
            if self.kernel_size[0]==1:
                offset[0, 2 * i + 0, 0, 0] = 0
                offset[0, 2 * i + 1, 0, 0] = offset_value[i]
            else:
                offset[0, 2 * i + 0, 0, 0] = offset_value[i]
                offset[0, 2 * i + 1, 0, 0] = 0
        # for i in range(self.in_channels):
        #     if self.kernel_size[0] == 1:
        #         offset[0, 2 * i + 0, 0, 0] = 0
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if self.mask:
            mask = torch.sigmoid(self.m_conv(input))
            mask = mask.contiguous().permute(0, 2, 3, 1)
            mask = mask.unsqueeze(dim=1)
            mask = torch.cat([mask for _ in range(input.size(1))], dim=1)
